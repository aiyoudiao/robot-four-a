 ...
 * @LastEditTime: 2020-04-05 15:31:23
 * @LastEditors: aiyoudiao
 * @FilePath: \project\store-server\src\common\MQ\MQConsumer.ts
 */
...
export default class RabbitMQ<T> {
  ...
  async receive<T>(hooks: (entity: T)=>boolean) {
    ...
    try {
      ...
      ch.consume(q.queue,async msg => {
        ...
        /* 调用一下钩子 */
        const result = await hooks(entity);
        
        if (result && badCount < 5) {
          badCount = 0;
         // 发送确认消息
         ch.ack(msg);
        } else {
