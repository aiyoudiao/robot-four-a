...
@store
@Form.create ()
class BotFriend extends Component {
  state = {
    users: [], //用户列表
  ...
  getUsers = async (page = 1) => {
    ...
    const res = await json.get ('/bot/friend/getFriends', {
      current: page - 1,
      weixin: fields.weixin || '', //koa会把参数转换为字符串，undefined也会
      name: fields.name || '',
      alias: fields.alias || '',
    });
    if (res.status !== 0) {
  ...
  showInfoModal = record => {
    ...
    const userInfo = {
      ...
      rTime: record.registrationTime &&
        moment (record.registrationTime).format ('YYYY-MM-DD HH:mm:ss'),
      ...
      lastLoginAddress: lastLoginAddress.ip &&
      ...
      // },
      {
        ...
        render: (text, record) => (
          <div style={{textAlign: 'left'}}>
            <span className="my-a" onClick={() => this.showInfoModal (record)}>
              <Icon type="eye" /> 查看
            </span>
            {this.props.user.username === record.username &&
              <Popconfirm
                title="您确定删除当前用户吗？"
                onConfirm={() => this.singleDelete (record)}
              >
                <span className="my-a">
                  <Divider type="vertical" /><Icon type="delete" /> 删除
                </span>
              </Popconfirm>}
          </div>
        ),
        ...
        <Card bordered={false}>
          ...
          <div style={{marginBottom: 16, textAlign: 'right'}}>
            <Button
              type="primary"
              icon="plus"
              onClick={() => this.toggleShowCreateModal (true)}
            >
              新增
            </Button>&emsp;
            <Button
